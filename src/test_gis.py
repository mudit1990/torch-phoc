...
def report_matches(outputs, embedding, matching, gis_data, original_words, k, length, is_lower):
    ...
    #print count
    return count, close_counts, matched_words, outputs, embedding, gis_data, qualified_ids_original
...
with open('../../../GIS_data/GIS_combined.txt') as f:
    gis_words = np.array(f.read().splitlines())
...
embedding = build_phoc_descriptor(words=gis_words,
...
for i in tqdm(range(0, len(A))):
 ...
 #    	words = np.array(new_words)
	...
	count, close_count, matched_words, new_outputs, new_embedding, new_word_strings, \
        qualified_ids = report_matches(outputs, embedding, 'cosine', gis_words, \
